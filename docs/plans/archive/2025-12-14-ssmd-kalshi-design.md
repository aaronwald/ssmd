...
impl ShardConfig {
    pub async fn resolve_symbols(&self, metadata: &MetadataClient) -> Result<Vec<Symbol>, Error> {
        // Query metadata for symbols matching selector
...
pub async fn start_connector(env: &str, shard: &str) -> Result<(), Error> {
    ...
    let symbols = config.resolve_symbols(&metadata).await?;
...
CREATE TABLE shard_assignments (
  ...
  symbol_id INTEGER REFERENCES markets(id),  -- or symbols table
  assigned_at TIMESTAMPTZ DEFAULT NOW(),

  UNIQUE(symbol_id)  -- Each symbol belongs to exactly one shard
);

CREATE TABLE shard_history (
  id SERIAL PRIMARY KEY,
  shard_id INTEGER REFERENCES shards(id),
  action VARCHAR(32) NOT NULL,  -- 'created', 'updated', 'reshard', 'deleted'
  old_config JSONB,
  new_config JSONB,
  reason TEXT,
  executed_by VARCHAR(64),
...
All components run with bounded memory (per design_IV.md requirement):
