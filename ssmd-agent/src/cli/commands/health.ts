...
export async function handleHealth(subcommand: string, flags: HealthFlags): Promise<void> {
  switch (subcommand) {
    case "trades":
      await runTradesCheck(flags);
      break;

    case "secmaster":
      await runSecmasterCheck(flags);
      break;

    case "daily":
      await runDailyHealthCheck(flags);
      break;
...
async function runTradesCheck(flags: HealthFlags): Promise<void> {
  const ticker = flags.ticker;
  if (!ticker) {
    console.error("Error: --ticker is required");
    console.log("Usage: ssmd health trades --ticker <TICKER> [--exchange kalshi] [--window 5m]");
    Deno.exit(1);
  }
  ...
  console.log(`Health Check: ${exchange}/${ticker}`);
...
async function fetchNatsTrades(
  ...
  const consumerName = `health-${Date.now()}`;
...
function printResults(result: HealthResult, detailed: boolean): void {
  console.log();
...
async function runSecmasterCheck(flags: HealthFlags): Promise<void> {
  ...
  console.log();
  console.log("=== Secmaster Health Check ===");
...
async function scoreConnectorFeed(
  ...
  // message_freshness: score based on how recent the last message is
  let freshnessScore = 0;
  if (lastTs) {
    const ageMs = Date.now() - new Date(lastTs).getTime();
    const ageMin = ageMs / 60000;
    if (ageMin < 5) freshnessScore = 100;
    else if (ageMin < 15) freshnessScore = 75;
    else if (ageMin < 60) freshnessScore = 50;
    else freshnessScore = 0;
  }

  // message_flow: messages > 0 and consumer count > 0
  const flowScore = (messageCount > 0 && consumerCount > 0) ? 100 : 0;

  const checks = [
    { name: "stream_has_data", weight: 0.40, value: streamHasData ? 100 : 0 },
    { name: "message_freshness", weight: 0.40, value: freshnessScore },
    { name: "message_flow", weight: 0.20, value: flowScore },
  ];
...
async function runDailyHealthCheck(flags: HealthFlags): Promise<void> {
  ...
  try {
    ...
    if (jsonOutput) {
      ...
      console.log();
      console.log(`Health Report: ${grade} (${composite}/100) — ${today}`);
...
function gradeColor(grade: string): string {
  return { GREEN: "#2e7d32", YELLOW: "#f9a825", RED: "#c62828" }[grade] ?? "#333";
}
...
function scoreBar(score: number): string {
  const color = score >= 85 ? "#2e7d32" : score >= 60 ? "#f9a825" : "#c62828";
  return `<div style="background:#e0e0e0;border-radius:4px;height:14px;width:120px;display:inline-block;vertical-align:middle">
    <div style="background:${color};border-radius:4px;height:14px;width:${Math.round(score * 1.2)}px"></div>
  </div> <strong>${score}</strong>/100`;
}

function buildHealthEmailHtml(report: DailyReport): string {
  const gc = gradeColor(report.grade);

  // Feed rows
  const feedRows = Object.entries(report.feeds)
    .filter(([name]) => !["completeness", "parquet-quality", "sla"].includes(name))
    .map(([name, feed]) => {
      const details: string[] = [];
      const d = feed as Record<string, unknown>;
      if (d.messageCount != null) details.push(`${fmtNum(d.messageCount as number)} msgs`);
      if (d.snapshots != null) details.push(`${d.snapshots} snaps`);
      if (d.products != null) details.push(`${d.products} products`);
      if (d.freshnessScore != null) details.push(`fresh: ${d.freshnessScore}`);
      if (d.lastFlushAge != null) details.push(fmtAge(d.lastFlushAge as number));
      return `<tr>
        <td style="padding:6px 8px;border-bottom:1px solid #eee">${escapeHtml(name)}</td>
        <td style="padding:6px 8px;border-bottom:1px solid #eee">${scoreBar(feed.score)}</td>
        <td style="padding:6px 8px;border-bottom:1px solid #eee;color:#666;font-size:12px">${details.join(", ")}</td>
      </tr>`;
    })
    .join("");

  // Phase 2 rows (if present)
  const phase2Feeds = ["completeness", "parquet-quality", "sla"];
  const phase2Rows = phase2Feeds
    .filter((name) => report.feeds[name])
    .map((name) => {
      const feed = report.feeds[name];
      return `<tr>
        <td style="padding:6px 8px;border-bottom:1px solid #eee">${escapeHtml(name)}</td>
        <td style="padding:6px 8px;border-bottom:1px solid #eee">${scoreBar(feed.score)}</td>
        <td style="padding:6px 8px;border-bottom:1px solid #eee;color:#666;font-size:12px"></td>
      </tr>`;
    })
    .join("");

  const issuesHtml = report.issues.length > 0
    ? `<h2 style="color:#c62828;font-size:16px;margin-top:24px">Issues</h2>
...
<body>
  <div class="container">
    <h1>
      <span style="color:${gc};font-size:24px">${report.grade}</span>
      Health Report — ${report.date}
      <span style="float:right;font-size:28px;color:${gc}">${report.composite}/100</span>
    </h1>

    <h2>Connector Health</h2>
    <table>
      <tr><th>Feed</th><th>Score</th><th>Details</th></tr>
      ${feedRows}
    </table>

    ${phase2Rows ? `<h2>Data Quality (GCS)</h2>
    <table>
    ...
    <div class="footer">
      Generated by ssmd health daily at ${new Date().toISOString()}
    </div>
  </div>
</body>
</html>`;
...
async function sendHealthEmail(report: DailyReport): Promise<void> {
  ...
  if (!user || !pass || !to) {
    console.error("[health] SMTP_USER, SMTP_PASS, and SMTP_TO required for email");
    return;
  }
  ...
  const html = buildHealthEmailHtml(report);

  await transporter.sendMail({
    from: user,
    to,
    subject: `[SSMD] Health ${report.grade} (${report.composite}/100) — ${report.date}`,
    html,
  });

  console.log(`Health email sent to ${to}`);
}
...
export function printHealthHelp(): void {
  console.log("Usage: ssmd health <command> [options]");
  console.log();
  console.log("Pipeline health checks for market data infrastructure");
  console.log();
  console.log("COMMANDS:");
  console.log("  daily           Composite health scoring: connector health + completeness + parquet quality + SLA");
  console.log("  trades          Compare NATS trades with exchange API");
  console.log("  secmaster       Run secmaster health checks");
  console.log();
  ...
  console.log("  ssmd health daily");
  console.log("  ssmd health daily --json");
