...
use crate::processor::{format_arrow_type, ManifestSchemaInfo, ParquetManifest, SchemaColumnDef};
...
mod tests {
    ...
    /// Massive (US equities) parquet is generated by parquet-gen under
    /// `massive/massive/massive/<date>/` with a `parquet-manifest.json`; it must
    /// be in FEEDS so the catalog (and `/v1/data/feeds`) indexes it. The
    /// feed/stream/prefix values must match the live GCS layout searched by
    /// `build_feed_summary` (`{prefix}/{prefix}/{stream}` = `massive/massive/massive`).
    #[test]
    fn feeds_contains_massive() {
        assert!(
            FEEDS
                .iter()
